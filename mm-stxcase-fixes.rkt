...
(define-metafunction mini
  ...
  [(eval (App ast_op ast_arg ...))
   (app (eval ast_op) ((eval ast_arg) ...))]
  [(eval val) val])

(define-metafunction mini
  app : val (val ...) -> val
  [(app (Fun var ast_body) (val_arg))
   (eval (subst ast_body var val_arg))]
  [(app prim (val_arg ...))
   (δ prim (val_arg ...))])
...
(define (preamble-env-pict)
  (define-metafunction mini
    [(preamble-env) ((lambda TFun)
                     (let-syntax TLet-Syntax)
                     (Quote TQuote) ;; otherwise, typesets as 'Quote :(
                     (syntax TQuote))])
